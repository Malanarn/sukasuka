...
public:
    ...
    void setNilai(int index, int val) {
        if (index >= 0 && index < MAKS_MAPEL)
            nilai[index] = val;
    }

    int getNilai(int index) const {
        if (index >= 0 && index < MAKS_MAPEL)
            return nilai[index];
        return 0;
    }
...
void tambahDataSiswa() {
    ...
    getline(cin, nama);
    daftarSiswa[jumlahSiswa].setNama(nama);

    for (int i = 0; i < jumlahMapel; i++) {
        int nilai;
        cout << "Nilai " << namaMapel[i] << ": ";
        cin >> nilai;
        daftarSiswa[jumlahSiswa].setNilai(i, nilai);
    }
...
void tampilkanData() {
    ...
    for (int i = 0; i < jumlahSiswa; i++) {
        cout << daftarSiswa[i].getNama() << ": ";
        for (int j = 0; j < jumlahMapel; j++) {
            cout << namaMapel[j] << "=" << daftarSiswa[i].getNilai(j);
...
void statistikMapel() {
    for (int i = 0; i < jumlahMapel; i++) {
        ...
        for (int j = 0; j < jumlahSiswa; j++) {
            int nilai = daftarSiswa[j].getNilai(i);
            total += nilai;
            if (nilai > max) {
                max = nilai;
                siswaMax = daftarSiswa[j].getNama();
            }
            if (nilai < min) {
                min = nilai;
                siswaMin = daftarSiswa[j].getNama();
            }
        }
...
void laporanSimulasi() {
    ...
    for (int i = 0; i < jumlahSiswa; i++) {
        int totalNilai = 0;
        file << daftarSiswa[i].getNama() << "\t";
        for (int j = 0; j < jumlahMapel; j++) {
            int nilai = daftarSiswa[i].getNilai(j);
            file << nilai << "\t";
            totalNilai += nilai;
        }
